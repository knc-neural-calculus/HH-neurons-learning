...
int main (int argc, char** argv) 
{
    ...
    double const learning_rate{10};
    int const mini_batch_size{1}, n_epochs{1200};
    ...
    PoissonSampler sampler(std::move(dataset.training_labels), 1);
    net.SGD(mini_batch_size, n_epochs, &sampler, true);
#else 
    NMNISTReader reader("../../N-MNIST-TRAIN/", std::move(dataset.training_labels));
    net.SGD(mini_batch_size, n_epochs, &reader, true);
